...
function renderTimeline(){{
  ...
  timelineChart=new Chart(ctx,{{type:'bar',data:{{labels:labels.map(l=>{{const d=new Date(l.replace(/-/g,'/'));return `${{d.getDate()}}.${{d.getMonth()+1}}.`;}}),datasets:[{{label:'Počet změn',data:values,backgroundColor:'#0052cc',borderRadius:4}}]}},options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}},tooltip:{{callbacks:{{title:(items)=>{{const w=labels[items[0].dataIndex];const d=new Date(w.replace(/-/g,'/'));return `Týden od ${{d.getDate()}}.${{d.getMonth()+1}}.${{d.getFullYear()}}`;}}}}}}},scales:{{x:{{grid:{{display:false}},ticks:{{maxTicksLimit:20}}}},y:{{beginAtZero:true}}}},onClick(evt){{const pts=timelineChart.getElementsAtEventForMode(evt,'nearest',{{intersect:true}},false);if(!pts.length)return;const ws=labels[pts[0].index];const we=new Date(ws.replace(/-/g,'/'));we.setDate(we.getDate()+6);const es=we.getFullYear()+'-'+String(we.getMonth()+1).padStart(2,'0')+'-'+String(we.getDate()).padStart(2,'0');const fF=document.getElementById('f-from');const fT=document.getElementById('f-to');if(fF.value===ws&&fT.value===es){{fF.value=DATA_META.dateFrom;fT.value=DATA_META.dateTo;}}else{{fF.value=ws;fT.value=es;}}applyFilters();}}}}}}  );
